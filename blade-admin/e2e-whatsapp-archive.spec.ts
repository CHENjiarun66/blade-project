...
test('聊天记录以只读 WhatsApp 视图展示文字与媒体完整性', async ({ page }) => {
  ...
  const initialScroll = await page.locator('.archive-message-canvas').evaluate(element => ({ top: element.scrollTop, max: element.scrollHeight - element.clientHeight }))
  ...
  await page.locator('.archive-message-canvas').evaluate(element => { element.scrollTop = 0; element.dispatchEvent(new Event('scroll')) })
  await expect(page.getByText('这是更早的历史消息')).toBeVisible()
  await expect(page.getByText('已显示 5 / 5 条')).toBeVisible()
  await expect.poll(() => page.locator('.archive-message-canvas').evaluate(element => element.scrollTop)).toBeGreaterThan(0)
