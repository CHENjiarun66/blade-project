...
test('聊天记录以只读 WhatsApp 视图展示文字与媒体完整性', async ({ page }) => {
  ...
  await page.route('**/api/whatsapp/bindings/confirmed', route => route.fulfill({ json: ok([{ id: 8, contactId: 18, contactName: 'Pastor（尼日利亚）', phoneNormalized: '2348033912244', customerId: 88, customerName: '尼日利亚客户', matchMethod: 'EXACT_PHONE', status: 'CONFIRMED', createTime: '2026-08-24T09:00:00', confirmedAt: '2026-08-25T10:00:00' }]) }))
  ...
  await page.route('**/api/whatsapp/archive/messages?*', route => {
    ...
    const records = requestedPage === '2' ? [
      { id: 1, sentAt: '2026-08-13T07:20:00', direction: 'INBOUND', messageType: 'TEXT', textContent: '这是更早的历史消息', starred: false, media: [] },
    ] : [
      { id: 2, sentAt: '2026-08-14T07:20:00', direction: 'INBOUND', messageType: 'TEXT', textContent: '请问这个产品还有库存吗？', starred: false, media: [] },
      { id: 3, sentAt: '2026-08-14T07:21:00', direction: 'OUTBOUND', messageType: 'IMAGE', textContent: '这是实拍图', starred: false, media: [{ id: 20, fileId: 1354, mediaType: 'IMAGE', mimeType: 'image/jpeg', downloadStatus: 'IMPORTED' }] },
      { id: 4, sentAt: '2026-08-14T07:22:00', direction: 'INBOUND', messageType: 'AUDIO', starred: false, media: [{ id: 30, fileId: 1355, mediaType: 'AUDIO', mimeType: 'audio/ogg', downloadStatus: 'IMPORTED' }] },
      { id: 5, sentAt: '2026-08-14T07:23:00', direction: 'OUTBOUND', messageType: 'VIDEO', starred: false, media: [{ id: 40, mediaType: 'VIDEO', downloadStatus: 'METADATA_ONLY', issueType: 'MEDIA_PATH_EMPTY' }] },
    ]
    return route.fulfill({ json: ok({ records, total: 5, size: 50, current: Number(requestedPage || 1), pages: 2 }) })
  })
  ...
  await expect(page.locator('.wa-message-image')).toHaveCount(1)
  await expect(page.locator('audio.wa-message-audio')).toHaveCount(1)
  ...
  await expect(page.getByText('向上滚动加载更早消息')).toBeVisible()
  const initialScroll = await page.locator('.wa-conversation__canvas').evaluate(element => ({ top: element.scrollTop, max: element.scrollHeight - element.clientHeight }))
  expect(initialScroll.max).toBeGreaterThan(0)
  expect(initialScroll.top).toBe(initialScroll.max)
  await expect(page.getByText('这是更早的历史消息')).toHaveCount(0)
  await page.locator('.wa-conversation__canvas').evaluate(element => { element.scrollTop = 0; element.dispatchEvent(new Event('scroll')) })
  await expect(page.getByText('这是更早的历史消息')).toBeVisible()
  await expect(page.getByText('已显示 5 / 5 条')).toBeVisible()
  await expect.poll(() => page.locator('.wa-conversation__canvas').evaluate(element => element.scrollTop)).toBeGreaterThan(0)
  await expect(page.getByRole('button', { name: /发送/ })).toHaveCount(0)

  await page.getByRole('tab', { name: /客户绑定/ }).click()
  await expect(page.getByText('绑定后，这个 WhatsApp 聊天会归入对应的 ERP 客户')).toBeVisible()
  await expect(page.getByText('已绑定客户', { exact: true })).toBeVisible()
  await expect(page.getByText('尼日利亚客户')).toBeVisible()
})
