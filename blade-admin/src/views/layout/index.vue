...
const navItems = computed<NavItem[]>(() => {
  const allItems: NavItem[] = [
    ...
    { name: '订单管理', path: '/orders', icon: 'shopping_bag', permission: 'menu:order', children: [
      ...
      { name: '草稿订单列表', path: '/orders/drafts' },
    ...
    { name: '档口管理', path: '/outlets', icon: 'storefront', permission: 'menu:outlet' },
...
const pageTitle = computed(() => {
  ...
  if (route.name === 'OrderDraftDetail') return '草稿订单详情'
  return titleMap[route.path] || 'Blade Admin'
})

const contentClass = computed(() => {
  return route.path === '/orders/quick' || route.name === 'OrderDraftDetail'
