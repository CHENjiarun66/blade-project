      ...
      <el-tab-pane label="聊天记录" name="archive">
        <div class="archive-shell">
          <aside class="archive-sidebar">
            ...
            <el-pagination v-model:current-page="archiveChatPage" small layout="prev, pager, next" :page-size="30" :total="archiveChatTotal" @current-change="loadArchiveChats" />
          </aside>
          ...
          <section v-if="selectedArchiveChat" class="archive-conversation">
            ...
            <div v-loading="archiveMessagesLoading" class="archive-message-canvas">
              <div class="archive-page-note">当前显示第 {{ archiveMessagePage }} 页 · 每页最多 50 条 · 只读归档</div>
            ...
            <footer class="archive-conversation-foot">
              ...
              <el-pagination v-model:current-page="archiveMessagePage" small layout="total, prev, pager, next" :page-size="50" :total="archiveMessageTotal" @current-change="loadArchiveMessages" />
            </footer>
          </section>
...
const archiveMessages=ref<ArchiveMessage[]>([]),archiveMessagePage=ref(1),archiveMessageTotal=ref(0),archiveMessagesLoading=ref(false)
...
async function loadAll(){loading.value=true;try{const [s,a,j,b]=await Promise.all([getIssueSummary(),getWhatsappAccounts(),getLatestWhatsappScan(),refreshBindingCandidates()]);summary.value=s.data||{...emptySummary};accounts.value=a.data||[];latestJob.value=j.data;bindings.value=b.data||[];if(!selectedAccount.value&&accounts.value.length)selectedAccount.value=accounts.value[0].id;await Promise.all([loadIssueChats(),loadInsights(),loadArchiveChats()]);if(issueDetailDrawer.value)await loadIssueDetails();startPollingIfNeeded()}finally{loading.value=false}}
...
async function selectArchiveChat(chat:ArchiveChat){selectedArchiveChat.value=chat;archiveMessagePage.value=1;await loadArchiveMessages()}
async function loadArchiveMessages(){if(!selectedArchiveChat.value)return;archiveMessagesLoading.value=true;try{const chat=selectedArchiveChat.value;const r=await getArchiveMessages({page:archiveMessagePage.value,size:50,accountId:chat.accountId,identityKey:chat.identityKey});archiveMessages.value=r.data.records;archiveMessageTotal.value=r.data.total}finally{archiveMessagesLoading.value=false}}
