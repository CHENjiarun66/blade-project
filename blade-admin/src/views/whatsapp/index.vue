    ...
    <el-tabs v-model="activeTab" class="rounded-2xl border border-slate-200 bg-white px-5 pt-2">
      <el-tab-pane label="聊天记录" name="archive">
        <div class="archive-shell">
          ...
          <WhatsappConversation v-if="selectedArchiveChat" :chat="selectedArchiveChat" class="archive-conversation" />
...
import { createCollector, decideBinding, decideWhatsappRecommendation, getArchiveChats, getConfirmedBindings, getInsightEvidence, getIssueChats, getIssueSummary, getIssues, getLatestWhatsappScan, getPendingBindings, getWhatsappAccounts, getWhatsappInsights, refreshBindingCandidates, requestWhatsappScan, type ArchiveChat, type BindingCandidate, type CollectionIssue, type CollectorCredential, type InsightEvidence, type IssueChat, type IssueSummary, type ScanJob, type WhatsappAccount, type WhatsappInsight } from '@/api/whatsapp'
...
function selectArchiveChat(chat:ArchiveChat){selectedArchiveChat.value=chat}
function searchArchive(){archiveChatPage.value=1;selectedArchiveChat.value=null;void loadArchiveChats()}
function resetArchive(){archiveChatPage.value=1;selectedArchiveChat.value=null;void Promise.all([loadArchiveChats(),loadIssueChats()])}
...
async function viewBoundChat(row:BindingCandidate){activeTab.value='archive';archiveKeyword.value=row.phoneNormalized||row.contactName||'';archiveChatPage.value=1;selectedArchiveChat.value=null;await loadArchiveChats()}
...
<style scoped>
.archive-shell{height:690px;display:grid;grid-template-columns:340px minmax(0,1fr);grid-template-rows:minmax(0,1fr);margin-bottom:20px;overflow:hidden;border:1px solid #dfe5e7;border-radius:16px;background:#f7f9f8}.archive-sidebar{display:flex;min-width:0;flex-direction:column;border-right:1px solid #dfe5e7;background:#fff}.archive-tools{display:grid;gap:10px;padding:14px;border-bottom:1px solid #edf0f1}.archive-search{font-size:12px;color:#168b6b}.archive-chat-list{min-height:0;flex:1;overflow:auto}.archive-chat-row{display:flex;width:100%;gap:12px;padding:13px 14px;border-bottom:1px solid #f0f2f3;transition:background .15s}.archive-chat-row:hover{background:#f4faf7}.archive-chat-row.is-active{background:#e9f7f1}.archive-avatar{display:grid;width:44px;height:44px;flex:0 0 44px;place-items:center;border-radius:50%;background:linear-gradient(145deg,#d9f4e9,#bce7d8);font-weight:700;color:#14765d}.archive-sidebar :deep(.el-pagination){justify-content:center;padding:10px;border-top:1px solid #edf0f1}.archive-conversation{border:0;border-radius:0}.archive-empty{display:grid;place-items:center;text-align:center;color:#7d8c85}.archive-empty span{font-size:50px}.archive-empty h3{margin-top:10px;font-weight:600;color:#36463f}.archive-empty p{margin-top:5px;font-size:13px}@media(max-width:900px){.archive-shell{grid-template-columns:280px minmax(0,1fr)}}
</style>
