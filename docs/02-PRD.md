...
- 多规格商品（两个及以上启用的真实 SKU）自动维护一个 `PLACEHOLDER`。后端编码保留 `{款号}-UNSPECIFIED-UNSPEC`（兼容历史 `{款号}-UNSPEC-UNSPEC`），页面统一显示为“整款录入（颜色/尺码未指定）”，不得要求用户理解技术编码。
- 无颜色且无尺码的商品自动创建一个 `DEFAULT`，后端编码保留 `{款号}-NA-NA`，页面统一显示为“无规格商品（实际 SKU）”。它是正常可售 SKU，不是占位 SKU。
- 只有一个启用真实/默认 SKU 时，款号查询直接匹配该 SKU，不额外启用占位 SKU。
