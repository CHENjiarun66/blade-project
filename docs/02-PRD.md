...
| 纸单批量草稿 | 本机识别 Agent 经可配置 NAS API 查询 SKU 候选并提交结构化结果，批量建立可人工复核的订单草稿；原图无需上传系统 |
...
- 本机 Agent 完成纸单图片识别和 Excel/结构化数据整理后，通过可配置的 NAS 生产 API 查询商品/SKU 候选并批量创建草稿，不上传原图、不直接访问数据库。当前外网入口为 `https://www.chenjianas.asia:33294`，客户端通过环境配置读取，不写死。
...
> 2026-09-04 架构调整：本章保留为历史方案。图片识别、Excel 整理、表格解析和初步款号判断由独立本机 Agent 承担；BladeProject 只负责结构化结果接收、商品候选、草稿复核和人工确认，不要求接收原图，也不再重复开发内置 OCR 页面与服务。
