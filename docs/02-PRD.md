...
| 纸单批量草稿 | 本机识别 Agent 经 NAS API 查询 SKU 候选、上传原图并批量建立可人工复核的订单草稿 |
...
- 本机 Agent 完成纸单图片识别和 Excel 汇总后，通过 NAS 生产 API 查询商品/SKU 候选并批量创建草稿，不直接访问数据库。
...
> 2026-08-27 架构调整：本章保留为历史方案。图片识别、Excel 整理、表格解析和初步款号判断由独立本机 Agent 承担；BladeProject 只负责原图接收、商品候选、草稿复核和人工确认，不再重复开发内置 OCR 页面与服务。
