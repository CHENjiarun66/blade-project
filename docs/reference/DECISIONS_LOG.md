...
| DEC-009 | 档口升级为主数据和数据权限边界 | 2026-09-20 | ✅ 已采纳 |
...
- 用 `sales_outlet` 管理稳定档口编码、名称、状态和默认档口。
- 用 `sys_user_outlet` 支持用户绑定一个或多个档口。
- 订单数据权限按“档口范围 × 人员范围”计算，不再只依赖 `btn:order:viewAll`。
- 正式订单保存 `source_outlet_id`，`source_shop` 仅保留名称快照。
- 统计、导出、文件和 Agent API 使用同一后端范围策略。

**核心理由**：

- 自由文本档口无法防止单据批次误写，也不能稳定统计。
- 老板、档口负责人和销售员需要不同的数据范围，而不是简单的全看/只看本人。
- 只在前端限制下拉选项不能阻止直接 API 越权。
- Agent Key 同样需要档口边界，业务 scope 不能替代数据范围。

**边界约束**：

- 档口不是仓库，不自动决定库存和发货仓。
- 每张订单只归属一个具体档口。
- 历史自由文本必须先审计和映射，不自动把“42”等疑似批次创建为档口。
- 详细方案以 [../20-OUTLET_ACCESS_CONTROL_DESIGN.md](../20-OUTLET_ACCESS_CONTROL_DESIGN.md) 为准。
