...
| BE-OUTLET-005 | `OrderAccessPolicy` 二维范围重构 | ✅ 完成（DeepSeek，2026-09-21；2026-09-21 完整性审计整改） | 订单列表、详情、动作、allowedActions 统一应用档口范围与人员范围；viewAll 不再独立绕过。完整性审计发现 `shipOrder`/占位拆分漏挂 `requireAccess`，已在第一批整改修复并补真实反例（`OrderShipScopeGuardTest`、`OrderPlaceholderSplitScopeGuardTest`、`CustomerOrderScopeIntegrationTest`） |
...
| TEST-OUTLET-001 | 后端越权矩阵 | 🚧 第一批整改完成（DeepSeek，2026-09-21） | 初次完整性审计发现 `shipOrder`、占位拆分、客户订单/统计/偏好三处旁路；已补 `OrderShipScopeGuardTest`（NONE 发货 403、幂等先鉴权、授权通过）、`OrderPlaceholderSplitScopeGuardTest`、`CustomerOrderScopeIntegrationTest`（档口×人员×缓存隔离×403）；其余 `OutletScopeMatrixTest`/`OrderOutletWriteRulesTest`/`AgentOutletScopeIntegrationTest` 保留。跨租户用户绑定/Agent Key 真实 DB 反例、NONE 用户/负责人全人员/改名快照等仍待第二批 |
