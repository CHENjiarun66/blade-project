...
| TEST-OUTLET-001 | 后端越权矩阵 | 🚧 第一批整改 + Codex 终审补强完成（DeepSeek，2026-09-21） | 初次完整性审计发现 `shipOrder`、占位拆分、客户订单/统计/偏好三处旁路；已补 `OrderShipScopeGuardTest`（NONE 发货 403、幂等先鉴权、授权通过）、`OrderPlaceholderSplitScopeGuardTest`、`CustomerOrderScopeIntegrationTest`（档口×人员×缓存隔离×403）；Codex 终审再补真实 DB/Redis 证据：`OrderScopeRealDbIntegrationTest`（真实 MySQL 下 403 且副作用为零）、`CustomerPreferenceCacheEvictionTest`（真实 Redis 下 evict 覆盖该客户全部范围/时间窗且不误删他人，含取消订单动作链路）；其余 `OutletScopeMatrixTest`/`OrderOutletWriteRulesTest`/`AgentOutletScopeIntegrationTest` 保留。跨租户用户绑定/Agent Key 真实 DB 反例、NONE 用户/负责人全人员/改名快照等仍待第二批 |
