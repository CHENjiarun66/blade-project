...
| BE-610 | 占位数量拆分到真实 SKU | ✅ 完成（执行人：ZCode；状态：WAITING_CODEX_FINAL_REVIEW） | 长任务系列 C；原子拆分并保持数量、金额和来源追溯守恒 |
| BE-611 | 占位 SKU 履约保护 | ✅ 完成（执行人：ZCode；状态：WAITING_CODEX_FINAL_REVIEW） | 长任务系列 C；未拆分占位 SKU 阻断配货、出库和库存调整 |
| BE-612 | 占位拆分审计与分析回算 | ✅ 完成（执行人：ZCode；状态：WAITING_CODEX_FINAL_REVIEW） | 长任务系列 C/E；拆分审计并防止占位与真实规格重复统计 |
