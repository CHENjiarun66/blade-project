...
| BE-590 | Agent 客户列表与详情敏感只读 | ✅ 完成 | `customers:read` 分页/单项读取客户名称、电话、地址、备注和订单数；Key 页面标记“敏感只读”，与脱敏订单读取完全分离 |
| BE-591 | Agent 客户新增与来源审计 | ✅ 完成 | `customers:create` 只允许新增客户；电话号码规范化并以重复电话返回 `DUPLICATE`，不覆盖旧客户；V60 分别记录创建 Agent Key 与用户来源 |
...
| BE-615 | 快速录单半成品暂存并续填 | ✅ 完成（Codex，2026-09-13） | 快速录单当前内容保存为 MANUAL 草稿且不跳页；重复点击更新同一草稿，草稿中心可续填完整字段并确认生成正式订单。V59 加法迁移保留 Agent 草稿兼容；后端 502/502、PC 构建及草稿关键路径 Playwright 3/3 通过，待生产发布 |
| BE-608 | 草稿确认转正式订单 | ✅ 完成 | JWT 人工确认后幂等创建正式订单；纸单数量/售价/总额/定金优先，草稿阶段不进入库存、财务和统计 |
| BE-616 | 草稿订单分页列表与批次工作台 | ✅ 完成（Codex，2026-09-14） | 左侧入口改为“草稿订单列表”，仅展示 EDITING 草稿并支持分页、批次/来源/待匹配/日期筛选；详情以 `/orders/drafts/{id}` 定位，按单据批次切换子单。CONFIRMED 仅后台保留作幂等和审计，确认后前台移除并可查看正式订单或继续本批次下一张；无需数据库迁移，待生产发布 |
| BE-617 | 录单批次/单号拆分与 SKU 首屏显示修正 | ✅ 完成（Codex，2026-09-15） | 快速录单和草稿详情把批次、单号设为独立必填字段；草稿按两个字段保存，正式订单兼容字段按 `批次_单号` 生成，下一单只递增单号。修复整款占位 SKU 乱码和草稿首次进入显示 `SKU {id}` 的加载竞态；无数据库迁移，待生产发布 |
| FE-618 | 订单详情加收金额一位小数与整元步进 | ✅ 完成（Codex，2026-09-15） | 加收金额默认 `0.0`，加减按钮按 1 元步进，允许手工输入 1 位小数；弹窗汇总和加收后预览统一显示 1 位小数。仅调整前端录入/显示，后端金额精度与历史数据不变，待生产发布 |
| FE-619 | 占位 SKU 配货弹窗语义与前置阻断 | ✅ 完成（Codex，2026-09-15） | 创建配货计划前发现整款录入/历史待明确规格即提示先拆分且不打开弹窗；遗留配货计划占位行统一显示中文业务语义，不暴露乱码或 `UNSPEC`。后端占位门禁不变；真实订单 1131 只读验证、Playwright 2/2、PC 构建通过，待生产发布 |
