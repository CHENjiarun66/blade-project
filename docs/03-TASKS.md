...
| AGENT-KEY-001 | Mac Agent Key 管理器与授权代理 | ✅ 完成，权限同步增强待发布 | 原生 macOS SwiftUI 应用；完整 Key 存系统钥匙串，附带白名单 MCP/命令代理。新增 `/api/agent/capabilities`，录入时自动同步真实 scope/到期时间，已有 Key 可手动同步，并区分本机元数据缺权限与服务器 403；源码与测试见 `tools/blade-agent-key-manager` |
