...
| 档口主数据与数据权限 | ⏳ 部分完成 | Series A–E 本地完成（V63–V68 迁移、后端统一策略、PC 管理端、订单/草稿/文件/看板/分析/客户统计/导出与 Agent 出口、租户上下文 fail-closed）；Series F 历史迁移/生产发布与 Series G 收口待外部 |
...
|------|------|------|------|------|------|
| WhatsApp 本地归档与客户工作区 | ✅ | ✅ | ✅ | ⏳ | V43-V47 已随 2026-09-11 release 部署，待 Mac Collector → NAS 真实联调 |
| 纸单 Agent 批量草稿 | ✅ | ✅ | ✅ | ⏳ | V48/V58 与前后端已部署，待 30 张真实纸单和最小 scope Key 验收 |
| SPU 占位 SKU 与分析隔离 | ✅ | ✅ | ✅ | ⏳ | V49/V50/V56 已部署；187 个商品均有唯一占位 SKU，待真实履约拆分验收 |
| 订单状态、金额、收款与履约重构 | ✅ | ✅ | ✅ | ⏳ | release `20260911_032005` 已迁移 145 单且全部门禁通过，待页面业务验收 |
...
| 2026-08-27 | Agent 商品候选、结构化批量订单草稿、人工确认和幂等处理 | 本地 MVP 与合成样本验证完成；原图上传后改为非必需兼容能力 |
...
| 2026-09-05 | Owner Agent Key 管理、一次性签发、轮换/停用与 Mac 外网配置 | V58、后端 API 和 PC 页签完成；待 NAS 生产联调 |
...
| P0-3 | TEST-PHASE2-002 | V48-V58 已部署；真实 Agent 联调依赖 P0-2 | 从 Mac 使用最小 scope Key 和可配置外网入口完成 30 单联调 |
...
| BE-135 | 订单编辑接口 | ✅ 完成 | PUT /api/orders/{id}；新状态机下仅“已确认 + 履约方式未决定 + 未收款/部分收款”允许修改客户、商品明细、数量、售价和运费，修改后应收不得低于当前净实收；已结清或进入履约后锁定结构，已发货/已完成仅允许备注、图片和受控档口变更 |
...
| BE-1052 | V42 至新版本迁移与 NAS 发布门禁 | ✅ 完成 | 2026-09-11 以 commit `12e1eb9`、release `20260911_032005` 完成 NAS V42→V58 发布；双份压缩备份验签通过，145 单迁移、0 人工核对、幂等重放新增 0，金额/状态/流水/SKU SQL 门禁全部通过。生产已恢复访问，新域名可信 HTTPS 200 |
| TEST-ORDER-LIFECYCLE-001 | 订单状态重构全链路验证 | ✅ 完成并终审通过 | 2026-09-11 后端全量 496/496、共享类型/PC/移动端构建通过；访问令牌不可充当刷新令牌，refresh token 采用 Redis 单次轮换，真实 HTTP 验证旧 JWT 在注销后由 200 变为 403；一次性空库验证 V1→V58、3 单迁移/零人工核对、幂等重放及全部 SQL 发布门禁；生产副本验证 V42→V58 与 145 单迁移/重放通过，当前最终候选已生成可留档证据 |
...
当前顺序：完成 `TEST-PHASE2-001` 真实纸单本地验收 → `TEST-PHASE2-002` 外网 30 单联调 → `ERP-DATA-001` → `ERP-PUR-STOCK-001`。完整财务保持后置。
...
| BE-409 | 客户商品偏好分析接口 | ✅ 完成 | GET /api/customers/{id}/preference，颜色/尺码/品类偏好统一按订单明细 quantity（购买件数）累计，不再按 SKU 行数计数；缓存版本升级并保留订单动作失效机制 |
...
| 任务 ID | 任务 | 状态 | 验收标准 |
|---------|------|------|---------|
| BE-415 | 订单记录分页 | ✅ 完成 | 后端支持 page/size 参数（默认1/20，最大100），返回 PageResult 含 total/pages；客户详情页已接入 10/20/50/100 分页并展示纸质单号、订单日期，108 单等大客户可完整翻页查看 |
| DATA-CUSTOMER-001 | 生产客户订单重复审计 | ✅ 审计完成，待人工处理 | API 分页无重复渲染、订单数量与明细数量一致；发现 Shakirah 两张、Gracemi 两张高度疑似历史重复订单，另有 JA 同纸质单号但日期不同记录；不自动删除，须备份后由业务逐单确认 |
...
| BE-587 | Agent 商品/订单脱敏读取 | ✅ 完成并发布（2026-09-16） | 增加 `products:read`、`orders:read`；商品、正式订单支持分页/详情，专用 DTO 排除成本、毛利、电话和地址；随 release `20260916_104900` 上线 |
| BE-588 | Agent 商品新增窄写入 | ✅ 完成并发布（2026-09-16） | `products:create` 只允许新增；同编码返回 DUPLICATE，不覆盖旧商品，不创建库存；随 release `20260916_104900` 上线 |
...
> 2026-09-11 更新边界：独立 Mac Agent 负责图片识别和 Excel/结构化数据整理；存在纸单图片时，通过本机 Key 管理器上传原图，再以 `sourceFileIds` 随结构化草稿关联。纯 Excel/原图缺失可降级但必须警告。外网 API 入口通过 Key 元数据配置，当前值为 `https://www.chenjianas.asia:33294`。

| 任务 ID | 任务 | 状态 | 备注 |
|---------|------|------|------|
| BE-601 | ERP 内置 OCR 图片上传 | ⏸ 转外部 Agent | ERP 不提供拍照识别入口；纸单原图由外部 Agent 经受控 source-files 接口上传 |
...
| BE-615 | 快速录单半成品暂存并续填 | ✅ 完成并发布（2026-09-16） | 快速录单当前内容保存为 MANUAL 草稿且不跳页；重复点击更新同一草稿，草稿中心可续填并确认；V59 随 release `20260916_104900` 上线 |
| BE-608 | 草稿确认转正式订单 | ✅ 完成（2026-09-24 客户口径统一） | JWT 人工确认后幂等创建正式订单；纸单数量/售价/总额/定金优先，草稿阶段不进入库存、财务和统计；已选客户直接关联，新客户名称+电话完整时按电话去重后自动建档，空白/散客不建档，信息不完整时阻断并提示；失效/跨租户 customerId 会解除关联后按同一规则重新解析 |
| BE-616 | 草稿订单分页列表与批次工作台 | ✅ 完成并发布（2026-09-16） | 列表只展示 EDITING 草稿并支持分页筛选；详情按批次切换子单；随 release `20260916_104900` 上线 |
| BE-617 | 录单批次/单号拆分与 SKU 首屏显示修正 | ✅ 完成并发布（2026-09-16） | 批次与单号独立必填；修复占位 SKU 乱码和详情首次加载标签；随 release `20260916_104900` 上线 |
| FE-618 | 订单详情加收金额一位小数与整元步进 | ✅ 完成并发布（2026-09-16） | 加收金额默认 `0.0`、整元步进并显示一位小数；随 release `20260916_104900` 上线 |
| FE-619 | 占位 SKU 配货弹窗语义与前置阻断 | ✅ 完成并发布（2026-09-16） | 占位行显示中文语义，未拆分时前后端共同阻止配货；随 release `20260916_104900` 上线 |
| ARCH-620 | 订单全流程回退、冲销与审批设计 | ✅ 设计完成（Codex，2026-09-15） | 新增 `18-ORDER_REVERSAL_APPROVAL_DESIGN.md`；锁定可逆/禁止状态、依赖门禁、R0-R3 风险等级、角色权限、审批职责分离、逆向事务和测试门禁；尚未开发 |
| BE-621 | 未出库履约回退与计划版本化 | ⏳ TODO | 实现待配货→已确认、配货中→待配货、待发货→配货中、仅记录已完成→已确认；取消配货停止物理删除计划和调整日志 |
| BE-622 | 订单回退申请与审批底座 | ⏳ TODO | 新增回退申请/审批日志、影响预览、R1/R2、申请人审批人分离、版本失效和幂等执行 |
| BE-623 | 出库冲销与库存反向流水 | ⏳ TODO | 已发货不得裸改状态；逐仓库/SKU 生成引用原出库的反向库存事实，并与退货、盘点和库存期间联动 |
| FE-620 | 订单回退按钮、影响预览与审批中心 | ⏳ TODO | 按后端 allowedActions 展示明确逆向动作；R0 二次确认，R1/R2 提交审批；PC 先行，移动端首版只读 |
| TEST-ORDER-ROLLBACK-001 | 订单回退、审批与冲销门禁 | ⏳ TODO | 覆盖状态矩阵、跨级拒绝、权限/数据范围、职责分离、审批失效、并发幂等、财务/库存守恒及生产副本预演 |
...
| BE-611 | 占位 SKU 履约保护 | ✅ 完成 | 长任务系列 C；未拆分占位 SKU 阻断配货、出库和库存调整 |
...
| 任务 ID | 任务 | 状态 | 备注 |
|---------|------|------|------|
| BA-201 | 订单列表页 | ✅ 完成 | 表格 + 高级筛选 + 分页 + 支付状态筛选 + 图片 fileId；列表只负责查找/浏览，操作列和行点击统一进入详情，不再保留重复编辑弹窗 |
| BA-202 | 订单详情页 | ✅ 完成 | 正式订单唯一编辑入口；按后端 `allowedActions.editOrder` 在“已确认 + 未进入履约 + 未收款/部分收款”显示“编辑订单内容”，集中修改单据基础信息、客户快照、商品/SKU、数量、售价、成本（受字段权限控制）、配送和运费，并阻止应收低于当前净实收；备注/图片与高权限档口变更分别独立操作，锁定订单仍可维护非结构信息 |
...
| BA-206 | PC 快速录单增强 | ✅ 完成 | 单张纸单连续录入、来源档口/店铺独立于仓库、订货/现货标记、运费收入/成本、成本与毛利快照；订单图片统一上传并在桌面端固定于右侧大图对照栏（缩放、缩略图切换、删除，最多 10 张），录入区保持独立滚动且不使用抽屉 |
...
### Phase 7.1: 档口主数据与跨档口数据权限（P0）

> 业务和架构依据：[20-OUTLET_ACCESS_CONTROL_DESIGN.md](./20-OUTLET_ACCESS_CONTROL_DESIGN.md)
> 实施顺序和验收门禁：[2026-09-20-outlet-access-control-rom-sow.md](./superpowers/plans/2026-09-20-outlet-access-control-rom-sow.md)
...
| BE-OUTLET-005 | `OrderAccessPolicy` 二维范围重构 | ✅ 完成（DeepSeek，2026-09-21；2026-09-21 完整性审计整改 + Codex 终审补强） | 订单列表、详情、动作、allowedActions 统一应用档口范围与人员范围；viewAll 不再独立绕过。完整性审计发现 `shipOrder`/占位拆分漏挂 `requireAccess`，已在第一批整改修复并补真实反例（`OrderShipScopeGuardTest`、`OrderPlaceholderSplitScopeGuardTest`、`CustomerOrderScopeIntegrationTest`）；Codex 终审再补真实 MySQL 反例 `OrderScopeRealDbIntegrationTest`（403 且订单状态/plan/inventory/transition/adjustment 副作用为零），并修正客户偏好缓存键为 `customer:preference:{customerId}:{scopeFingerprint}:{start}:{end}` 使前缀 evict 可命中 |
| BE-OUTLET-006 | 草稿档口权限接入 | ✅ 完成（DeepSeek，2026-09-21） | 草稿列表/批次/详情/更新/确认统一范围；selectForUpdate 后复核；空档口确认阻断 |
| BE-OUTLET-007 | 订单/草稿 DTO 与名称快照 | ✅ 完成（DeepSeek，2026-09-21；第二批B 审计兜底） | 正式订单必须有具体档口；`sourceOutletId/code/name` 返回；服务端主数据名称快照；改档口高权限 + 原因 + 审计（V66）；列表结构化筛选。第二批B：`applyOutletChange` 在任何 order/log 写入前取得可靠 User，无可靠 principal 一律 401 fail-closed，不伪造 1L、不依赖 `order_outlet_change_log.operator_id NOT NULL` 数据库异常兜底 |
| BE-OUTLET-008 | 看板与分析范围接入 | ✅ 完成（DeepSeek，2026-09-21；第二批A 补入口权限） | Dashboard/Analytics 所有订单型指标统一 `OrderReadScope`（档口 × 人员），未归档默认排除；当前无缓存，新增范围指纹防未来错误缓存；Agent 公共路径同样按 Key 范围裁剪；第二批A 补服务端 `@PreAuthorize('menu:dashboard')`/`@PreAuthorize('menu:analytics')`，无权限 403、有权限可用，不依赖前端菜单隐藏（`DashboardAnalyticsPermissionIntegrationTest`） |
| BE-OUTLET-009 | 导出与文件权限接入 | ✅ 完成（DeepSeek，2026-09-21；第二批A 列表/详情一致性修复） | 订单导出 + 文件中心/订单/草稿图片全出口：`FileBusinessAccessPolicy`、PUBLIC/previewToken 业务校验、多绑定 ALL、SQL 预分页、no-store；第二轮 Codex 整改新增 `FileRequestContext` 去掉 tenant=1/user=1 回退、afterCommit 用文件租户、清理任务多租户遍历（708/708）；Agent 出口留 E3；第二批A 修复 `buildVisibilityCondition`：`data:order:peopleAll` 无 `btn:file:viewAll` 时不得把 `create_by` 占位写成 null，本人未绑定文件 list/count/detail 必须一致可见，订单/草稿绑定仍按档口范围 |
...
| BA-OUTLET-003 | 订单与草稿档口选择器 | ✅ 完成（DeepSeek，2026-09-21；Codex 2026-09-23 交互收口） | 单档口锁定、多档口只显示授权集合、Owner 显示全部启用档口；快速录单/新建/草稿详情接入；正式订单档口变更移至详情页独立高权限弹窗，必须选择不同档口并填写审计原因，不与普通内容编辑混合 |
...
| BA-806 | 草稿国家区号与客户录入口径统一 | ✅ 本地完成（Codex，2026-09-24） | 修复国家区号点击后被二次切换关闭；下拉挂载到页面顶层并支持搜索选择。草稿确认与快速录单统一：已有客户关联、新客户自动建档、空白按散客；待生产发布 |
...
| 任务 ID | 任务 | 状态 | 备注 |
|---------|------|------|------|
| BA-901 | PC 订单图片上传接入 | ✅ 完成 | 新建/快速录单/详情页接入统一文件接口；详情页“维护备注/图片”独立支持图片墙上传/预览/移除，上传先进入文件中心，订单保存时同步 fileId 绑定；生命周期锁定后仍可维护 |
| BA-902 | PC 商品主图上传接入 | ✅ 完成 | 商品新建/编辑/列表接入统一文件接口，商品保存 fileId |
| BA-903 | PC 入库凭证上传接入 | ✅ 完成 | 入库弹窗接入统一文件接口，入库日志保存 fileId |
| BA-904 | PC 图片上传 E2E 回归 | ✅ 完成 | Playwright 覆盖商品主图、订单详情“维护备注/图片”、快速录单图片上传，断言统一文件接口返回 fileId、预览图可加载且无系统错误 |
...
| 任务 ID | 任务 | 状态 | 备注 |
|---------|------|------|------|
| BA-1111 | Agent Key 管理页面 | ✅ 完成 | 系统管理新增 Owner 专属 Agent Key 页签；支持创建、查看前缀/scope/有效期/最近使用、不可逆停用和轮换；完整密钥只显示一次，并可按可配置 `BLADE_AGENT_API_BASE_URL` 复制 Mac 环境配置 |
