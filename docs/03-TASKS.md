...
| BE-624 | 草稿订单单条删除与批量删除 | ✅ 完成本地开发（待发布） | 列表支持勾选和批量删除，详情支持删除当前草稿；仅允许 `EDITING` 草稿且要求 `btn:order:delete`，整批预校验后逻辑删除主表/明细并解除图片绑定，原图保留在文件中心未绑定区；后端 533/533、PC 构建、Playwright 2/2 通过 |
