...
| BE-1052 | V42 至新版本迁移与 NAS 发布门禁 | ⏳ 进行中（执行人：Codex） | 2026-09-10 已对 NAS V42 生产副本完成 145 单 V42→V58 迁移/幂等重放，0 人工核对，数量与金额对账一致，并为当前最终候选提交生成非敏感 PASS 证据，生产未写入。发布工具已补齐不可变镜像、备份、维护页、迁移及 SQL 门禁，并改为 NAS TLS 密钥只读挂载。仍待可信外网 TLS 及用户批准维护窗口；候选 commit 再变更则证据自动失效 |
| TEST-ORDER-LIFECYCLE-001 | 订单状态重构全链路验证 | ✅ 完成并终审通过 | 2026-09-10 后端全量 496/496、共享类型/PC/移动端构建通过；访问令牌不可充当刷新令牌，refresh token 采用 Redis 单次轮换，真实 HTTP 验证旧 JWT 在注销后由 200 变为 403；一次性空库验证 V1→V58、3 单迁移/零人工核对、幂等重放及全部 SQL 发布门禁；生产副本验证 V42→V58 与 145 单迁移/重放通过，当前最终候选提交已生成可留档证据 |
