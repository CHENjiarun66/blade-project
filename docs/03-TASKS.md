...
>
> 本阶段由其他 Agent 实现，Codex 不认领编码任务，只负责 `CR-0`～`CR-8` 架构和代码审核。实现 Agent 必须按[执行看板](./superpowers/plans/2026-08-30-order-refactor-agent-execution-board.md)先完成只读 `ORDER-SOW-0`，每个工作包提交 `WAITING_CODEX_REVIEW` 后暂停。
