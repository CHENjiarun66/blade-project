...
> 2026-09-04 最终边界：独立 Mac Agent 负责图片识别和 Excel/结构化数据整理；BladeProject 只接收结构化 JSON，或接收标准 Excel 后转换为同一结构，负责商品候选、草稿、人工复核和正式订单转换。纸单原图不要求上传系统。外网 API 入口通过 `BLADE_AGENT_API_BASE_URL` 配置，当前值为 `https://www.chenjianas.asia:33294`。
