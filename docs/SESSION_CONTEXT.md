...
- 正式上线剩余硬门禁：为 `frp-pen.com:33294` 配置包含正确 SAN 的可信 TLS（当前是 `CN=blade` 自签证书）；基于最终 release commit 重跑 V42→V58 生产副本预演并留存 `result=PASS/manual_review=0` 证据；用户批准维护窗口后才允许执行发布脚本。
