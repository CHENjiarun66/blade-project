...
## 2026-09-25 草稿客户口径与 Agent SKU 自动匹配（已发布生产）

- 修复国家区号组件点击触发与手动显示相互冲突，下拉现在可稳定展开、搜索和选择，并挂载到页面顶层避免被草稿布局遮挡。
- 草稿确认客户规则统一到快速录单：已有客户关联；新客户名称+电话完整时按电话去重并自动建档；空白/散客不建档；部分信息阻断并提示补齐或清空。
- 客户解析收敛到后端 `OrderDraftCustomerResolver`，因此网页手工草稿和 Agent 导入草稿确认时行为一致；订单仍保留当次客户快照。
- Agent 只识别到款号时：精确且唯一的商品会自动落到启用的“整款录入”占位 SKU；歧义、无占位 SKU 或已给出具体颜色/尺码但无法精确匹配时保持待匹配，不做猜测。
- V69 将正式订单客户电话扩为 `varchar(50)`，仅扩容、不改写历史数据，保证国际国家区号和格式符可从草稿完整进入正式订单。
- 验证：PC build 通过；草稿确认与自动匹配 Playwright 均通过；真实 MySQL 隔离基线完成 V68→V69，后端全量 **868/868**（Failures 0 / Errors 0 / Skipped 0），空库 Flyway V1→V69 通过。
- 生产：commit `83312c4fee03e0e393e5500bdb72bcd8935a254a`，release `20260925_draft_customer_83312c4`；Flyway V69，172 张订单/752 条明细不变，迁移与重放新增 0，五项订单门禁为 0；MySQL/Redis 未重启，可信外网 HTTPS 200，维护模式已解除。

## 2026-09-21 档口最终收口：客户 orderCount 范围、V68 默认唯一、移动端错误去重
...
- 残留 P2：WhatsApp 分析订单事实口径待产品确认；无 FK（按项目约定）；V68/V67 DDL 大表锁表时长需生产副本实测。
...
- 统一入口：`TenantContext.requireTenantId()` 缺失时抛业务 403“缺少租户上下文”；`TenantLineHandler.getTenantId()` 调用该入口，**彻底移除默认 tenant=1**。业务写入口另在方法首行主动 require，保证稳定 403 而非底层包装异常。`OutletServiceImpl.requiredTenantId()` 也从 401 统一为 403。
...
- 正式订单编辑入口已收口到订单详情页：列表仅查找/浏览并统一进入详情；“编辑订单内容”负责单据基础信息、客户、商品、配送和金额，“维护备注/图片”在订单锁定后仍可使用，“变更来源档口”是需独立权限与原因审计的高权限操作。
