...
| 下一步 | 订单库存软解耦生产口径已完成本地收口：MySQL 8 V1-V39 累计迁移、后端全量测试、PC 构建和浏览器关键路径均已通过；下一步进入 `develop` 集成，后续 release 仍需按发布规范做生产备份和 NAS 发布预检。部分发货、分批发货和缺货退款继续排除。 |
...
- `TEST-ORDER-INV-001` 已完成：MySQL 8 临时库 V1-V39 累计迁移通过；后端全量 `mvn test` 383 项通过；PC `npm run build` 通过；浏览器关键路径覆盖 UI 登录、订单创建、定金、追加收款、抹零结清、配货计划、确认调整、发货和详情页渲染。
- 仪表盘数据权限尚未实现。
