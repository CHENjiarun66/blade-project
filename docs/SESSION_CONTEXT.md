...
- 新增缺失媒体恢复需求：后续 V44 `wa_collection_issue` + BE-572/BA-1101 工作台展示 Mac 端未下载/不完整媒体，用户加载后重扫并自动转为已恢复；Mac 单端无法检测完全未落地的 iPhone 消息。
- `BE-570` Mac Collector v0.1 已完成并在真实源只读运行：989 个私聊、48,244 个源行归并为 32,035 条逻辑消息，2,133 个媒体可用、14,993 个媒体路径为空；本地 CSV 已生成，尚未上传 ERP。采集器独立仓库提交 `f7b5551`。
- 下一步优先以 v0.1 spool 契约实现 `BE-567` 导入鉴权/批次 API，再做 BE-569 消息媒体导入和 BE-572/BA-1101 完整性工作台。
