...
| 项目名称 | BladeProject |
| 启动日期 | 2026-03-21 |
| 当前阶段 | 后端核心模块、PC 管理端主要业务页面、库存并发控制、跨仓总量预留、配货计划、权限基础能力、订单编辑和追加收款均已落地；统一文件上传和文件中心底座已完成；WhatsApp 本地归档与客户工作区已完成；纸单识别 Agent 的 SKU 候选、批量订单草稿、草稿工作台和人工确认正式订单 MVP 已于 2026-08-27 完成本地验证；移动端继续开发中 |
| 下一步 | 先由用户在本地 `/orders/drafts` 复核纸单草稿工作台和真实操作习惯，再调整字段与匹配体验；确认后按 NAS 发布手册配置生产 Agent Key、部署 V48 和前后端。仪表盘数据权限、移动端真实数据接入和 Agent Gateway 其他未完成能力继续排期。 |
