...
- `BE-1052` 发布工具已实现但生产步骤未执行：要求干净 Git、同 commit 生产副本预演证据、不可变镜像、维护页、压缩备份+SHA-256+NAS 外副本、历史迁移执行与幂等重放、迁移后 SQL 不变量校验。
- 生产仍保持 Flyway V42 且未写入。正式上线剩余硬门禁：修复 `frp-pen.com:33294` 的可信 TLS；基于最终 release commit 重跑 V42→V58 生产副本预演并留存 `result=PASS/manual_review=0` 证据；用户批准维护窗口后才允许执行发布脚本。
