...
| 下一步 | 图片派生图已在本机测试环境完成历史补生成和业务验收；生产环境仍须按发布规范备份后分租户、小批执行。后续继续商品管理 v2（SKU 精细维护、商品图集、SKU 图片、删除引用保护）、订单库存解耦收尾（BE-124、BE-126）、看板完善（仪表盘数据权限）、外部 Agent 只读 Gateway（BE-551~BE-565）、移动端页面、OCR 识别服务与拍照录单；Catalog 后续可补客户身份、行为采集、选款清单、公开分享/只读专用账号 |
