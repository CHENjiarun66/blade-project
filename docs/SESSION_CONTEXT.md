...
| 下一步 | 图片派生图已在本机测试环境完成历史补生成和业务验收，当前进入 `develop` 集成与生产发布准备；生产环境仍须按发布规范备份、预检原图路径后分租户、小批执行。后续继续订单库存解耦收尾（BE-124、BE-126、BE-138~BE-140）、看板数据权限、外部 Agent 只读 Gateway 剩余能力、移动端页面、OCR 识别服务与拍照录单；Catalog 后续可补客户身份、行为采集、选款清单、公开分享/只读专用账号 |
