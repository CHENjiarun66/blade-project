...
1. 第一版代码应预留 `FileDerivativeService`、`ImageDerivativeGenerator`、`FileStorageProvider` 这类边界接口，避免缩略图逻辑散落在 Controller 或业务模块中。
2. `variant_type` 不要写死在前端页面中，应由前端统一 URL 工具和后端枚举/常量管理，方便后续增加 `video_cover`、`webp_card`、`avif_card` 等类型。
3. 本地存储第一版可以同步生成派生图，但服务层要按“可异步化”设计：生成失败可记录状态，后续可替换为队列、重试任务或独立 worker。
4. 派生图存储路径必须仍通过存储 Provider 生成，不允许业务代码拼接本地路径；后续切 NAS、七牛云、CDN 时只改 Provider 和配置。
5. 历史补生成任务第一版可先做手动触发或管理入口，但表结构和状态字段需要能表达 `PENDING`、`READY`、`FAILED`，方便后续批量重试和运维排查。
6. 脚手架只做扩展点，不做过度实现：第一版不接真实 CDN、不做多格式自适应、不做复杂队列，只保证后续接入时不需要重构业务表和图片展示调用链。
...
3. 图片上传后生成 `thumb`、`card` 两种派生图。
4. 新增 `/api/files/{id}/variant?type=thumb|card` 接口，权限复用原图预览权限。
5. PC 文件中心、商品列表、订单图片墙优先使用派生图。
6. Catalog 卡片和缩略图条优先使用派生图，全屏大图仍使用原图。
