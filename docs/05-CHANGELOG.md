...
- 草稿订单列表增加多选列、单条删除和批量删除；草稿详情页增加“删除草稿”。所有删除操作均需要二次确认，并由 `btn:order:delete` 控制前端可见性和后端访问权限。
- 新增 `DELETE /api/order-drafts/{id}` 与 `POST /api/order-drafts/batch-delete`。只允许删除 `EDITING` 草稿；已确认并生成正式订单的草稿必须保留幂等和审计记录，不能删除。批量删除最多 100 张，先校验整批存在性和状态，任一草稿不符合条件时整批不执行。
- 删除采用 MyBatis 逻辑删除，同步逻辑删除草稿商品明细并解除 `order_draft` 文件绑定；纸质单原图不会物理删除，而是保留到文件中心“未绑定”区，降低误删风险。
- 新增删除成功、已确认草稿拒绝、部分草稿不存在时整批拒绝的后端反例测试；后端全量 `533/533`、PC 生产构建及草稿列表 Playwright `2/2` 通过。当前完成本地开发，尚未发布生产。
