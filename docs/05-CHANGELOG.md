...
- Agent SKU 结构分析把占位销量计入款号总量，真实颜色尺码排名排除占位，新增未指定汇总、规格覆盖率与数据质量等级。
- 对外商品目录和有库存商品判断排除占位 SKU，避免把款号级销售容器当作真实库存。
- 验证：后端完整测试集 399/399 通过；`blade-admin npm run build` 通过；本地 MySQL V49/V50 执行成功且 90 个多规格商品对应 90 个唯一占位 SKU。
