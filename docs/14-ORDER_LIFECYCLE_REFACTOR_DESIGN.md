...
V59 起草稿以 `entry_source` 区分 `AGENT` 与 `MANUAL`：Agent 草稿中的 `deposit` 是纸单识别值或人工确认值，并继续使用纸单总额优先；快速录单暂存的手工草稿使用 `paid_amount`，订单应收按商品明细加客户运费计算。两类金额都只在草稿转正式订单时写入首笔正式收款流水，草稿阶段不进入财务和经营统计。
