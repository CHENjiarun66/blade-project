...
> **2026-08-30 执行方式调整**：用户决定减少逐阶段文档确认。自基线 `d800ec4` 起，本项目改按 [Z Code 长任务](./2026-08-30-order-refactor-zcode-long-run-task.md) 连续实施。原 SOW-1～SOW-7 和 CR-1～CR-7 继续作为范围、自测和提交检查点，但 Z Code 完成一个系列后无需等待 Codex 放行，可直接进入下一系列；全部完成并自测后，由 Codex 对完整 Diff 做一次最终审核。生产写入、release、NAS 发布和用户批准门禁不变。
