...
> 状态：Series A–D 已完成；Series E1（统计/仪表盘/导出 + 统计筛选）完成（DeepSeek，2026-09-21）；E2/E3、F、G 待开发
>
...
| BE-OUTLET-009 | 导出与文件权限接入 | 🚧 导出完成（DeepSeek，2026-09-21） | 订单导出已接统一读范围 + 显式档口/待归档校验；订单/草稿图片绑定与预览留 E2 |
...
| TEST-OUTLET-003 | 全出口防泄漏回归 | ⏳ TODO | 列表、详情、统计、导出、文件、Agent、缓存键和跨角色切换（E2/E3） |
