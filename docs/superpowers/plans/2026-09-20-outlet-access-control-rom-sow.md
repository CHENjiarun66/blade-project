...
| 任务 ID | 任务 | 状态 | 交付物 |
|---|---|---|---|
| BE-OUTLET-007 | 订单/草稿 DTO 与写入规则 | ⏳ TODO | `sourceOutletId/code/name`；服务端生成名称快照；禁止批次兜底 |
| BA-OUTLET-003 | 订单和草稿档口选择器 | ⏳ TODO | 快速录单、新建订单、草稿详情；单档口锁定、多档口授权选择 |
| BA-OUTLET-004 | 列表筛选与历史待归档提示 | ⏳ TODO | 订单/草稿按档口筛选；Owner 可查看“待归档档口” |
| TEST-OUTLET-002 | 录单与草稿 E2E | ⏳ TODO | 手工草稿、Agent 草稿、草稿确认、正式订单和越权选择 |
...
| 任务 ID | 任务 | 状态 | 交付物 |
|---|---|---|---|
| BE-OUTLET-008 | 看板与分析范围接入 | ⏳ TODO | 汇总、趋势、排行和客户贡献先裁剪档口/人员范围再聚合 |
| BE-OUTLET-009 | 导出与文件权限接入 | ⏳ TODO | 订单导出、草稿/订单图片绑定和预览范围与详情一致 |
| BE-OUTLET-010 | Agent Key 档口范围 | ⏳ TODO | Key 签发/轮换、capabilities、档口查询和草稿/订单接口校验 |
| BA-OUTLET-005 | 统计档口筛选 | ⏳ TODO | Owner 多档口筛选和对比；普通用户只能选择授权集合 |
| BA-OUTLET-006 | Agent Key 档口配置 | ⏳ TODO | 全部/指定档口、默认档口和服务器能力同步展示 |
| TEST-OUTLET-003 | 全出口防泄漏回归 | ⏳ TODO | 列表、详情、统计、导出、文件、Agent、缓存键和跨角色切换 |
