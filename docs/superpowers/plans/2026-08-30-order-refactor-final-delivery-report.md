...
## 十四、收款动作语义拆分补充（2026-08-31）

### 14.1 修正原因与最终契约

订单详情原“确认收款”把输入值传给按“本次新增金额”处理的兼容接口，界面却将其标为“实收金额”，存在部分收款后重复累计的风险。现将两个意图彻底拆开：

| 动作 | 用户输入 | 财务结果 | 是否结清 |
|---|---|---|---|
| 加收金额 | 本次新增收款 | 只新增一笔 `RECEIPT` | 由余额自然判断，不做短款核销 |
| 确认收款 | 最终累计实收金额 | 新增差额 `RECEIPT`，剩余差额记 `WRITE_OFF` | 是 |

结清计算以当前有效应收为边界，而非固定商品标价：

```text
当前有效应收 = 当前净实收 + 当前尾款
本次新增收款 = 最终累计实收 - 当前净实收
短款核销金额 = 当前有效应收 - 最终累计实收
```

例如当前有效应收 500 元、已收 400 元：最终实收填 500 元时新增收款 100 元、短款 0 元；填 495 元时新增收款 95 元、短款核销 5 元。短款必须填写原因并要求 `btn:order:writeOff` 权限。

### 14.2 实施范围

- 后端新增 `ConfirmSettlementDTO`、`POST /api/orders/{id}/confirm-settlement` 和锁内计算服务；幂等键覆盖本次结清动作。
- PC 与移动端订单详情均提供独立“加收金额”“确认收款”入口和实时金额预览。
- 保留 `/confirm-payment` 作为旧客户端兼容入口，其金额仍代表本次新增收款；新页面不再使用。
- 未增加数据库字段或 migration；复用既有 `RECEIPT`、`WRITE_OFF` 流水及财务快照。

### 14.3 安全与验收规则

1. 最终累计实收不得小于当前净实收，也不得超过当前有效应收。
2. 短款核销金额大于 0 时，必须填写原因并通过核销权限校验。
3. 没有任何正数实收时，不允许把整单直接作为短款核销。
4. 最终差额由服务端在订单行锁内重新计算，前端预览不作为入账依据。
5. 本次只在本地重构分支及隔离测试环境验证，未连接 NAS/生产；发布门禁仍按第 13.4 节执行。

### 14.4 验证结果

| 验证项 | 结果 |
|---|---|
| 新增结清金额单测 | 4/4 通过：足额只追加差额、短款拆分收款与核销、拒绝低于当前实收、拒绝超过有效应收 |
| 后端全量测试（隔离库 `127.0.0.1:3307/blade_project`） | **471/471 通过，0 失败，0 错误，0 跳过** |
| Flyway | 57 个 migration 校验通过，隔离库保持 V55；本次无需新增 migration |
| `packages/types` | `npm run build` 通过 |
| `blade-admin` | `npm run build` 通过；仅既有 chunk 大小提示 |
| `blade-mobile` | `npm run build` 通过；仅既有 Vite 配置/chunk 提示 |
| 本地预览 | PC 前端 `http://127.0.0.1:5777` 与后端 `http://127.0.0.1:8081` 已加载本次代码 |
